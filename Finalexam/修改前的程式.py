...
# 設定參數
import micropython
Version = micropython.const("main_v13")
VersionTime = micropython.const("2022/09/04 04:00")
...
def showValues(i=0, msg=''):
    if hasOLED:
        ...
        oled.text('Air Hum : {:.1f} %'.format(airrh), 0, 41)
...
def sendMQTT(msg=''):
    ...
    if msg=='':
        payload = "field1="+str(airTemp)+"&field2="+str(airrh)+"&field3="+str(soilTemp)+"&field4="+str(soilMois)+"&field5="+str(isWatering)+"&field6="+str(isWaterScarcity)
    else:
        payload = msg
...
while True:
    ...
    # 讀取 DHT22 空氣溫溼度
    (airTemp, airrh) = readDHT()
    print('DHT22 Temperature  : {}{}C'.format(airTemp, '\u00b0'))
    print('DHT22 Humidity     : {}%'.format(airrh))
